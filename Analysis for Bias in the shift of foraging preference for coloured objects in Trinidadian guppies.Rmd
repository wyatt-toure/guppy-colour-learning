...
<!--chapter:end:02-project-1-methods.Rmd-->
...
![Objects from a very early experiment](images/objects.jpg)

```{r, include=FALSE}

pilot.data = read.csv('Exp2_Shifting_Pref.csv')

pilot.data$trial = as.factor(pilot.data$trial)

pilot.data = pilot.data %>% mutate(time.with.trained.object = case_when(object.side.etho.view == "R" ~ time.with.right.object,
                                                            object.side.etho.view == "L" ~ time.with.left.object))

pilot.data = pilot.data %>% mutate(time.with.untrained.object = case_when(object.side.etho.view == "L" ~ time.with.right.object,
                                                              object.side.etho.view == "R" ~ time.with.left.object))

pilot.data = pilot.data %>% mutate(rewarding.object.preference = (time.with.trained.object-time.with.untrained.object))

pilot.data = pilot.data %>% mutate(rewarding.object.preference.prop = (time.with.trained.object/(time.with.trained.object+time.with.untrained.object)))
...
```{r, results=FALSE,echo=FALSE}
tidy.fit.pilot.data = broom.mixed::tidy(pilot.model)


tidy.fit.pilot.data$p.value = ifelse(tidy.fit.pilot.data$p.value < .001, ## Condition
                                           "< .001",  ## if condition is true
                                           paste(tidy.fit.pilot.data$p.value %>% round(3)))

tidy.fit.pilot.data[10:16,] %>% select(-group,-effect)
```

```{r, results=TRUE, echo=FALSE}
knitr::kable(tidy.fit.pilot.data[10:16,] %>% select(-group,-effect))

allEffects(pilot.model)
```




<!--chapter:end:03-project-1-results.Rmd-->
...
<!--chapter:end:Analysis-for-Bias-in-the-shift-of-foraging-preference-for-coloured-objects-in-Trinidadian-guppies.Rmd-->
