...
subtitle: "Experiment 1 Analysis"
author: 
  - name: "M. Wyatt Toure"
...
biases in learned foraging preferences in Trinidadian guppies'. The raw data
used to conduct these analyses are available in the
...
my_data <- my_data %>%
  group_by(id) %>%
  mutate(prop.rewarding.object.preference = (time.with.trained.object/(time.with.trained.object + time.with.untrained.object)))
...
#### ESM Model 4 - Preference for the rewarded object during training based on feeding
...
effect along with individual ID in the model.
  ...
  - **Response variable:** `rewarding.object.preference` is the time
...
# Saving plot to figs directory
ggsave(
  filename = "exp-1-ESM-model-4-residual-plot.png",
  plot = (plot(simulationOutput)),
  path = "figs/exp-1/residual-plots/",
  device = "png",
  dpi = 300
...
ggsave(
  filename = "exp-1-ESM-model-4-training-data-ate-plot.png",
  path = "figs/exp-1/",
  device = "png",
  dpi = 300
)
...
Below we provide a list of packages used, their versions, and references.
