...
[Methods](methods.html) page. Note the code blocks that produce the figures and
tables are not shown on this page as they are rather long, however the code to
produce the figures and tables can also be seen in
[analysis.Rmd](https://github.com/wyatt-toure/guppy-colour-learning-project/blob/main/analysis-experiment-2.Rmd).
...
At the time experiment 1 was conducted I still performed much data preparation
manually in Excel so unlike in experiment 2, we do not format the data in R for
experiment 1. Additionally, several variables were created in Excel. This Excel
sheet was then exported as the `.csv` file
[`colour-learning-project-data.csv`](https://github.com/wyatt-toure/guppy-colour-learning/blob/main/data/colour-learning-project-data.csv).
All measures in
[`colour-learning-project-data.csv`](https://github.com/wyatt-toure/guppy-colour-learning/blob/main/data/colour-learning-project-data.csv)
except for `ate` are derived from the first 16 columns of that data sheet.
...
`r report_est_and_std_error(tidy_baseline_model, rounding = 2)` seconds, p =
...
rewarded objects by
`r report_est_and_std_error(tidy_training_data_model, rounding = 2)[3]` seconds
...
`r report_est_and_std_error(tidy_training_data_model, rounding = 2)[2]` seconds.
However, there was no significant interaction effect between rewarding object
colour and trial
(`r report_est_and_std_error(tidy_training_data_model, rounding = 2)[4]`
...
In addition to the above effects, to control for heterogeneous variance across
...
`r report_est_and_std_error(tidy_test_data_model, rounding = 2)[4]` seconds
...
(`r report_est_and_std_error(pairwise_comparison_contrasts_table, 2, F)[4]`
...
`r report_est_and_std_error(pairwise_comparison_contrasts_table, 2, F)[2]`
...
`r report_est_and_std_error(pairwise_comparison_contrasts_table, 2, F)[1]`
...
preference:
`r report_est_and_std_error(pairwise_comparison_contrasts_table, 2, F)[3]`
...
arising from feeding count variation we included feeding count as a co-variate
and re-ran the analysis in [Model 3](#model-3).
...
# Saving plot to figs directory
ggsave(
  filename = "exp-1-ESM-model-3-residual-plot.png",
  plot = (plot(simulationOutput)),
  path = "figs/exp-1/residual-plots/",
  device = "png",
  dpi = 300
...
We find the same pattern of results as in [Model 3](#model-3)
...
section of the README. Below we provide a list of packages used, their versions,
and references.
