...
script and find that the statistical conclusions are maintained. Additionally, 
two fish died before testing in Experiment 1 and were not included in the 
analyses.
...
[`emmeans`](https://cran.r-project.org/package=emmeans). We compared the
following means:
...
Additionally, we compared the mean for each colour group in each test trial
against zero to see if the preference for the rewarding object was significantly
...
contrast_against_chance <- as.data.frame(
  emmeans(test_data_model_glm,
    specs = ~ rewarding.object.colour:trial,
    adjust = "mvt"
  ) %>%
...
contrast_against_chance$p.value <- 
  format_p_value(contrast_against_chance$p.value)

contrast_against_chance$trial <- c(
  "Initial Test",
  "Initial Test",
  "Final Test",
  "Final Test"
...
contrast_against_chance %>%
  ...
  kable(caption = "Comparisons of estimated marginal means of rewarding object preference by rewarding object colour for every test trial in Experiment 1 against zero. A multivariate t adjustment was used to address multiple comparisons.")
```

We find that only green-trained guppies during the final test trial display a
rewarding object preference that is significantly different from chance (green
object preference: `r contrast_against_chance$emmean[4] %>% round(0)` ±
`r contrast_against_chance$SE[4] %>% round(0)` seconds, p
`r contrast_against_chance$p.value[4]`).
...
# Add the feeding values to the main data frame so we can get treatment IDs
...
Additionally, we again compared each test trial mean to chance (50% in this 
case). 
...
percent_contrast_against_chance <- as.data.frame(
  emmeans(prop_test_data_model_glm,
    specs = ~ rewarding.object.colour:trial,
    adjust = "mvt",
    type = "response"
...
percent_contrast_against_chance$trial <- c(
  "Initial Test",
  "Initial Test",
  "Final Test",
  "Final Test"
...
percent_contrast_against_chance$p.value <- 
  format_p_value(percent_contrast_against_chance$p.value)

percent_contrast_against_chance %>%
  rename(
    "Rewarding object colour" = rewarding.object.colour,
    "Trial" = trial,
    "Estimate" = response,
  ...
  kable(caption = "Comparisons of estimated marginal means of rewarding object preference by rewarding object colour for every test trial in Experiment 1 against chance (50%). A multivariate t adjustment was used to address multiple comparisons.")
```

We find that only green-trained guppies during the final test trial display a 
rewarding object preference that is significantly different from chance 
(green object preference: `r percent_contrast_against_chance$response[4] * 100` ± `r percent_contrast_against_chance$SE[4] * 100`%, p `r percent_contrast_against_chance$p.value[4]`) which is the same result we find when
using a difference measure.
