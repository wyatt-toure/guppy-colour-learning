...
This script will generate figures to store them we need to have specific
...
### ESM Model 1 - Including feeding count as a covariate {#esm-model-2}
...
\@ref(tab:feeding-controlled-table) is the output feeding controlled model. Below
we have the output table for the non-feeding-count controlled model from
[Model 3](#model-3) (Table \@ref(tab:model-3-table)) reproduced. In both models
the statistical conclusions are the same.
