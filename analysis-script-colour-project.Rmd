...
# Setting table row names
test.model.table.row.name.vec = c("Intercept", "Test type", "Rewarding object colour", "Rewarding object colour X Test type")
...
```{r, results=TRUE, echo=FALSE}
knitr::kable(tidy_test.data.model[1:4,] %>% dplyr::select(-group, -effect) %>% mutate_if(is.numeric, round, digits = 3), caption = "Table of post-hoc tests with a multivariate-t adjustment for multiple comparisons of a selected set of means. The numbers represent the initial test (‘Initial’) and the final test (‘Final’). The colour corresponds to the identity of the rewarding object (blue for blue-rewarded guppies, green for green-rewarded guppies). Values are all rounded to 3 decimal places. Significant p-values are bolded. CL = confidence limit.")
```
