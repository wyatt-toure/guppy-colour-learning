...
knitr::kable(tidy_fit0%>% mutate_if(is.numeric, round, digits = 3))
```
...
```{r,  results=TRUE, echo=FALSE}
knitr::kable(tidy_fit[2:4,] %>% dplyr::select(-group, -effect)%>% mutate_if(is.numeric, round, digits = 3))
```
...
objects_image = magick::image_read('images/objects.jpg')
...
```{r, results=TRUE, echo=FALSE}
knitr::kable(tidy_fit.feeding.data.model[2:2,]%>% mutate_if(is.numeric, round, digits = 3))
```
