...
other for the analysis on this data set.
...
near the green versus the blue object across all guppies (p =
...
heterogeneous variance across trials we additionally modeled the variance due
to trial. 
...
model_3_anova_table %>% 
  ...
  mutate_if(is.numeric, round, digits = 3) %>% 
  rename("P Value" = 4) %>% 
  kable(caption = "ANOVA table for the test trials")
```

There is an interaction effect between trial and rewarding object colour for the
test trials (p = `r model_3_anova_table$'Pr(>Chisq)'[4] %>% round(3)`). To
investigate the interaction effect and determine where the differences in
...
and the test trial was significant. We used the Dunnett adjustment method to
...
test_data_contrasts %>%
  ...
  kable(
    caption = "Table of post-hoc tests with a Dunnett adjustment for multiple 
...
`r test_data_contrasts$p.value[1]`) and green-trained guppies significantly
increased their preference for the rewarding object by
...
to a Lego object with a different blue and green colouration (blue: p =
`r test_data_contrasts$p.value[2]`, green: p =
`r test_data_contrasts$p.value[7]`).

Green-trained guppies showed evidence of generalizing their learned preference
to a non-Lego clay object (p = `r test_data_contrasts$p.value[8]`), displaying a
change in preference of `r test_data_contrasts$estimate[8] %>% round(0)` seconds
whereas blue-trained guppies did not show evidence of a learned preference (p =
...
`r test_data_contrasts$p.value[4]`).
...
seconds (p `r test_data_contrasts$p.value[10]`) but blue-trained guppies did not
(p = `r test_data_contrasts$p.value[5]`) displaying a non-significant shift of
`r test_data_contrasts$estimate[5] %>% round(0)` seconds.
...
After trial 21 the guppies were all weighed. These can be seen in the code chunk
below where we programatically assign the weights.

```{r}
## Assigning weights
weight_data <- full_data %>%
  filter(trial == 0) %>% 
  mutate(
    weight =
      case_when(
...
grams) but this difference was not statistically significant.
...
With the proportional data we again find an overall interaction effect between
trial and rewarding object colour (p =
...
preference of 21% whereas blue-trained guppies did not show evidence of a learned
preference (p = `r prop_test_data_contrasts$p.value[3]`).
...
preference of 25% (p = `r prop_test_data_contrasts$p.value[10]`) but blue-trained
guppies did not (p = `r prop_test_data_contrasts$p.value[5]`).
...
object_pair_model_anova_table %>% 
  ...
  mutate_if(is.numeric, round, digits = 3) %>% 
  rename("P Value" = 4) %>% 
  kable(caption = "ANOVA table for object pair model")
```
...
```{r, object-pref-training-plot, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Relative preference for the rewarding object for all guppies during training trials (trials 1-20). Large circles are means ± 95% CI, smaller circles are raw data for each individual across all training trials", fig.id="object-pref-training-plot"}
ggplot(
  training_data,
  aes(
    x = object.pair,
...
However, we note that object pair 4 is the only object pair to have 4 out of 5
of its presentations after the halfway mark of training (trial 10, Figure
\@ref(fig:training-object-pair-plot)). This means guppies were much more
...
```{r, training-object-pair-plot, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Relative preference for the rewarding object for all guppies during training trials (trials 1-20). Negative values represent more time spent with the unrewarding object, positive values indicate more time spent with the rewarding object. Light grey lines connect individuals across trials. Large coloured circles are trial means ± 95% CI."}
ggplot(
  training_data,
  aes(
    x = trial,
...
ggsave(
  filename = "exp-2-esm-training-object-pair-plot.png",
  path = "figs/exp-2/",
  device = "png",
  dpi = 300
)
