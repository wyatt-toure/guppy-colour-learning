...
ggplot(
  test_data %>% filter(trial == "0" | trial == "21"),
  aes(
    x = trial,
  ...
  scale_color_manual(values = c("#4e79a7", "#59a14f")) +
  scale_shape_manual(values = c(15, 16)) +
...
```{r, echo=FALSE}
ggplot(
  test_data %>% filter(trial == "23" | trial == "25"  | trial == "21" | trial == "0" | trial == "27" | trial == "29") %>% filter(id != "8b"), 
  aes(
    x = trial, 
    y = rewarding.object.preference, 
    colour = rewarding.object.colour,
...
names(object_labels) <- c("blue", "green")
ggplot(
  test_data %>% filter(trial == "21" | trial == "0"),
  aes(
    x = trial,
