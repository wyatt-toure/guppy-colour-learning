...
After trial 21 all guppies were weighed. In the next chunk of code we
...
set `showWarnings` to `FALSE`. Every figure seen on this page will be available
as an individual file in the `figs/` directory if one runs all the code chunks
...
preference: `r report_est_and_std_error(tidy_baseline_model, rounding = 0)`
...
colour (`r report_est_and_std_error(tidy_training_data_model, rounding = 0)[4]`
...
training_data_model_trends_values$p.value <- 
  format_p_value(training_data_model_trends_values$p.value)
...
[@hothorn2008SimultaneousInferenceGeneral]. Since the multivariate t
adjustment is simulation based we set a random seed to produce consistent
confidence intervals upon re-running the code.
...
Finally we compared the mean for each colour group in each test trial against 
zero to see if the preference for the rewarding object was significantly 
different from zero within a test trial.
...
```{r contrasts-against-zero, echo=FALSE}
set.seed(123)

contrast_against_zero <- as.data.frame(
  emmeans(test_data_model_glm,
    specs = ~ rewarding.object.colour:trial,
    adjust = "mvt"
  ) %>%
...
contrast_against_zero$p.value <- format_p_value(contrast_against_zero$p.value)

contrast_against_zero$trial <- c(
  "Initial",
  "Initial",
  "Probe 1",
  "Probe 1",
...
contrast_against_zero %>%
  ...
  kable(caption = "Comparisons of estimated marginal means of rewarding object preference by rewarding object colour for every test trial in Experiment 2 against zero. A multivariate t adjustment was used to address multiple comparisons.")
```
...
`r report_ci(test_data_contrasts, rounding = 0)[1]`) and green-trained guppies
...
`r report_ci(test_data_contrasts, rounding = 0)[6]`). There is no difference in
performance between the two colour treatments in this test (p =
`r test_data_simple_contrasts$p.value[2]`,
`r report_ci(test_data_simple_contrasts, rounding = 0)[2]`). Green-trained
guppies have a rewarding object preference that differs significantly from zero
(p = `r contrast_against_zero$p.value[4]`) but blue-trained guppies do not (p =
`r contrast_against_zero$p.value[3]`).
...
`r report_ci(test_data_contrasts, rounding = 0)[2]`, green: p =
`r test_data_contrasts$p.value[7]`,
`r report_ci(test_data_contrasts, rounding = 0)[7]`) and the groups did not
perform differently on this test (p = `r test_data_simple_contrasts$p.value[3]`,
`r report_ci(test_data_simple_contrasts, rounding = 0)[3]`). Neither
green-trained nor blue-trained guppies have a rewarding object preference that
differs significantly from zero: (green: p =
`r contrast_against_zero$p.value[6]` ; blue: p =
`r contrast_against_zero$p.value[5]`).
...
`r report_ci(test_data_contrasts, rounding = 0)[8]`), displaying a change in
...
`r report_ci(test_data_contrasts, rounding = 0)[3]`). There is a significant
...
`r report_ci(test_data_simple_contrasts, rounding = 0)[4]`). Green-trained
guppies have a rewarding object preference that differs significantly from zero
(p = `r contrast_against_zero$p.value[8]`) but blue-trained guppies do not (p =
`r contrast_against_zero$p.value[7]`).
...
`r report_ci(test_data_contrasts, rounding = 0)[9]`) but blue-trained guppies
did not (p = `r test_data_contrasts$p.value[4]`,
`r report_ci(test_data_contrasts, rounding = 0)[4]`). Green-trained guppies do
not significantly outperform blue-trained guppies in this test (p =
`r test_data_simple_contrasts$p.value[5]`,
`r report_ci(test_data_simple_contrasts, rounding = 0)[5]`). Neither
green-trained nor blue-trained guppies have a rewarding object preference that
differs significantly from zero (green: p =
`r contrast_against_zero$p.value[10]`; blue: p =
`r contrast_against_zero$p.value[9]`).
...
`r report_ci(test_data_contrasts, rounding = 0)[10]`) but blue-trained guppies
did not (p = `r test_data_contrasts$p.value[5]`,
`r report_ci(test_data_contrasts, rounding = 0)[5]`) displaying a
...
`r report_ci(test_data_simple_contrasts, rounding = 0)[6]`). Green-trained
guppies have a rewarding object preference that differs significantly from zero
(p `r contrast_against_zero$p.value[12]`) but blue-trained guppies do not (p =
`r contrast_against_zero$p.value[11]`).

Thus, in three out of five post-training test trials green-trained guppies
showed evidence of learning while blue-trained guppies show evidence of learning
in only one out of five test trials with this trial being the very first probe
trial after training. Additionally, in three out of five post-training test
trials, green-trained guppies show a preference for their rewarding object that
is significantly different from chance whereas blue-trained guppies never
display a rewarding object preference that is significantly different from
chance. Moreover, in two out of five test trials green-trained guppies
outperform blue-trained guppies. Meanwhile blue-trained guppies never outperform
green-trained guppies.
...
Finally we compared the mean for each colour group in each test trial against 
chance to see if the preference for the rewarding object was significantly 
different from 50% within a test trial.

```{r, echo=FALSE}
set.seed(123)

percent_contrast_against_zero <- as.data.frame(
...
percent_contrast_against_zero$trial <- c(
  "Initial",
  "Initial",
  "Probe 1",
  "Probe 1",
...
percent_contrast_against_zero %>%
  ...
  kable(caption = "Comparisons of estimated marginal means of rewarding object preference by rewarding object colour for every test trial in Experiment 2 against chance (50%). A multivariate t adjustment was used to address multiple comparisons.")
```
...
```{r object-pair-plot, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Object-pair preferences during Experiment 2 (pair 1: red squares; pair 2: green circles; pair 3: blue triangles; pair 4: purple diamonds). (A) Relative preference for the rewarding object for all guppies across all training trials. Large points are means ± 95% CI, smaller points are raw data points. To the right of the raw data are probability density functions displaying the distribution of the data. (B) Relative preference for the rewarding object for all guppies during each individual training trial (trials 1-20). Positive values indicate more time spent with the rewarding object and negative values represent more time spent with the unrewarding object. Light grey lines connect individuals across trials. Large coloured points are trial means ± 95% CI, smaller points are raw data points.", fig.height=8.5, fig.width=7.5}
p1 <- ggplot(
  training_data,
  aes(
    x = object.pair,
...
[metadata](https://github.com/wyatt-toure/guppy-colour-learning#metadata)
section of the README.

```{r, r-packages, echo=FALSE}
report(sessionInfo()) %>% 
  as.data.frame() %>% 
  kable(caption = "All packages used for this analysis. The dependencies for these packages as well as their versions can be found in the metadata.md file.")
```
