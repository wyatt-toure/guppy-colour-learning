...
For the main effects of **training** and **rewarding object colour** on object preference I first fit a **linear mixed effects model** with **fixed effects of trial** and **rewarding object colour** and a random effect of **individual id**. Due to heterogeneity in the variance among grouops however, I switched to a generalized linear mixed model with a gaussian distribution which also modelled the variances the vaiances to account for variance heterogeneity.  My third model asks whether the preference for the rewarding object changed between baseline and final test and looks for an interaction with rewarded object colour.
...
test.data.model.pairwise.comparisons = 
  emmeans(test.data.model, specs = ~ trial:rewarding.object.colour)
