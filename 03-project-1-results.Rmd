...
tidy_fit.test.data.model[2:4,] %>% dplyr::select(-group, -effect)
```


```{r, results=TRUE, echo=FALSE}
knitr::kable(tidy_fit.test.data.model[2:4,] %>% dplyr::select(-group, -effect))
```
...
knitr::kable(pairwise.comparison.contrasts.table.show %>% dplyr::select(contrast, estimate, df, lower.CL,upper.CL,p.value),
...
```{r, results=TRUE, echo=FALSE}
knitr::kable(tidy_fit.test.feeding.data.low.feeders.removed[2:4,] %>% dplyr::select(-group, -effect))
```
...
```{r, results=TRUE, echo=FALSE}
knitr::kable(tidy_fit.test.data.feeding.controlled.model[2:5,]%>% dplyr::select(-group, -effect))
```
